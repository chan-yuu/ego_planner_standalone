...
namespace ego_planner
{
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    for (int i = order_; i <= i_end; ++i)
    {
      ...
      {
        Eigen::Vector3d pos = (a * init_points.col(i - 1) + (1 - a) * init_points.col(i)).eval();
        occ = grid_map_->getOccupancy(pos);
        // cout << setprecision(5);
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      ...
      {
        std::cerr << "[ERROR] a star error, force return!" << std::endl;
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        ...
        if (got_intersection_id >= 0)
        {
          ...
          if (length > 1e-5)
          {
            ...
            {
              Eigen::Vector3d pos = ((a / length) * intersection_point + (1 - a / length) * cps_.points.col(j)).eval();
              occ = grid_map_->getOccupancy(pos);
      ...
      else
      {
        ...
        // std::cerr << "[ERROR] " <<("Failed to generate direction! segment_id=%d", i);
      }
    }
  ...
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      bool occ = grid_map_->getOccupancy(pos);
      ...
      if (occ)
      {
        ...
        for (j = i - 1; j >= 0; --j)
        {
          Eigen::Vector3d pos_j = cps_.points.col(j);
          occ = grid_map_->getOccupancy(pos_j);
          if (!occ)
          {
            in_id = j;
            break;
        ...
        {
          std::cerr << "[ERROR] " <<("ERROR! the drone is in obstacle. This should not happen.");
          in_id = 0;
        }

        for (j = i + 1; j < cps_.size; ++j)
        {
          Eigen::Vector3d pos_j2 = cps_.points.col(j);
          occ = grid_map_->getOccupancy(pos_j2);
        ...
        {
          std::cout << "[WARN] " <<("WARN! terminal point of the current trajectory is in obstacle, skip this planning.");
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        {
          std::cerr << "[WARN] a star error in rebound, skip this segment" << std::endl;
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        if (got_intersection_id >= 0)
        {
          ...
          std::cout << "[WARN] " <<("Failed to generate direction. It doesn't matter.");
      }
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    constexpr int MAX_RESART_NUMS_SET = 10;  // 增加到10次重启
    do
    {
      /* ---------- prepare ---------- */
      ...
      {
        //std::cout << "[WARN] " <<("Solver error in planning!, return = %s", lbfgs::lbfgs_strerror(result));
        flag_force_return = false;
        ...
        for (double t = tm; t < tmp * 2 / 3; t += t_step) // Only check the closest 2/3 partition of the whole trajectory.
        {
          ...
          flag_occ = grid_map_->getOccupancy(pos);
          if (flag_occ)
          {
            ...
            if (t <= bspline_interval_) // First 3 control points in obstacles!
            {
              ...
              std::cout << "[WARN] " <<("First 3 control points in obstacles! return false, t=%f", t);
              return false;
            }
      ...
      {
        std::cout << "[WARN] Solver error. Return = " << result << ", " << lbfgs::lbfgs_strerror(result) << std::endl;
        // while (ros::ok());
      }

    } while ((flag_occ && restart_nums < MAX_RESART_NUMS_SET) ||
             (flag_force_return && force_stop_type_ == STOP_FOR_REBOUND && rebound_times <= 50));  // 增加到50次

    if (!success) {
        std::cout << "[BsplineOptimizer] Optimization failed after " << restart_nums << " restarts and " << rebound_times << " rebounds" << std::endl;
    }
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      {
        std::cerr << "[ERROR] " <<("Solver error in refining!, return = %d, %s", result, lbfgs::lbfgs_strerror(result));
      }
